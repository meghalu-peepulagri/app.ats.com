...
const _B84BZJ = eventHandler((event) => {
...
async function A({ request: n, event: t }) {
  ...
  const h = n.method, g = new URL(n.url, "http://localhost:3000"), x = new RegExp(`${L$1("/_server")}/([^/?#]+)`), y = g.pathname.match(x), o = y ? y[1] : null, c = Object.fromEntries(g.searchParams.entries()), m = "createServerFn" in c, E = "raw" in c;
  ...
  const N = ["multipart/form-data", "application/x-www-form-urlencoded"], f = await (async () => {
    try {
      let e = await (async () => {
        ...
        })) return T(h.toLowerCase() !== "get", "GET requests with FormData payloads are not supported"), await a(await n.formData(), i);
      ...
      return e.result instanceof Response ? e.result : !m && (e = e.result, e instanceof Response) ? e : isRedirect(e) || isNotFound(e) ? R(e) : new Response(e !== void 0 ? startSerializer.stringify(e) : void 0, { status: getResponseStatus(getEvent()), headers: { "Content-Type": "application/json" } });
    } catch (e) {
      return e instanceof Response ? e : isRedirect(e) || isNotFound(e) ? R(e) : (console.info(), console.info("Server Fn Error!"), console.info(), console.error(e), console.info(), new Response(startSerializer.stringify(e), { status: 500, headers: { "Content-Type": "application/json" } }));
    }
  })();
...
const X = defineHandlerCallback(async ({ request: e, router: t, responseHeaders: a }) => {
  if (typeof u.renderToReadableStream == "function") {
    ...
    const o = transformReadableStreamWithRouter(t, n);
    return new Response(o, { status: t.state.statusCode, headers: a });
  }
  if (typeof u.renderToPipeableStream == "function") {
    const n = new PassThrough();
    try {
      const s = u.renderToPipeableStream(jsx(_, { router: t }), { ...isbot(e.headers.get("User-Agent")) ? { onAllReady() {
        s.pipe(n);
      } } : { onShellReady() {
        s.pipe(n);
      } }, onError: (c, r) => {
        console.error("Error in renderToPipeableStream:", c, r);
      } });
    } catch (s) {
      console.error("Error in renderToPipeableStream:", s);
    }
    const o = transformPipeableStreamWithRouter(t, n);
    return new Response(o, { status: t.state.statusCode, headers: a });
  }
...
async function L(e, t, a) {
  ...
  if (isPlainObject$1(o) && o.method) {
    const r = o, p = r.data instanceof FormData ? "formData" : "payload", f = new Headers({ ...p === "payload" ? { "content-type": "application/json", accept: "application/json" } : {}, ...r.headers instanceof Headers ? Object.fromEntries(r.headers.entries()) : r.headers });
    if (r.method === "GET") {
      const i = encode$1({ payload: startSerializer.stringify({ data: r.data, context: r.context }) });
...
export { trapUnhandledNodeErrors as a, useNitroApp as b, destr as d, ee as e, setupGracefulShutdown as s, toNodeListener as t, useRuntimeConfig as u, w };
